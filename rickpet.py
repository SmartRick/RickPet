...
# 宠物类
class RickPet():
    def __init__(self, path, x, y, width=None, height=None, minimum=None, auto_scale=False):
        # 宠物ID
        ...
        if auto_scale != True:
            ...
            # 宽度
            self.width = width
        self.minimum = 80 if minimum == None else minimum
        # 图像刷新间隔毫秒数
        self.gap = 200
        # 当前刷新图像的下标
        self.index = 0
    ...
    def init_clothes(self, path):
        if os.path.exists(path) and os.path.isdir(path):
            ...
            for file in listdir:
                ...
                if self.auto_scale:
                    self.width = q_image.width()
                    self.height = q_image.height()

                self.clothes.append(q_image)
                print(file_path)
        else:
...
class PetGui(Draggable, RightButtonChangeSize):
    ...
    def init_gui(self):
        ...
        # self.showFullScreen()
        self.setGeometry(self.pet.x, self.pet.y, self.pet.width, self.pet.height)
    ...
    def paintEvent(self, event):
        ...
        image = QPixmap.fromImage(self.pet.clothes[self.pet.index])
        scaled = image.scaled(self.geometry().size(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation)
        q_painter.drawPixmap(0, 0, scaled)

        # q_painter.end()

    def mousePressEvent(self, event):
        super(PetGui, self).mousePressEvent(event)
        if event.button() == Qt.RightButton:
            self.pos = QCursor().pos()
            self.current_size = self.geometry()
            # print(min(self.current_size.size().width(),self.current_size.size().height()))
            self.size_changeing = True

    def mouseReleaseEvent(self, event):
        super(PetGui, self).mouseReleaseEvent(event)
        self.size_changeing = False

    def mouseMoveEvent(self, event):
        super(PetGui, self).mouseMoveEvent(event)
        if self.size_change:
            change_value_temp = -(self.pos.x() - QCursor().pos().x())
            change = min(self.current_size.size().width(), self.current_size.size().height()) - change_value_temp
            print(change)
            if change <= self.pet.minimum:
                return
            else:
                change_value = change_value_temp
            # print(change_value)
            self.resize(self.current_size.width() - change_value, self.current_size.height() - change_value)
            self.pet.width = self.current_size.width() - change_value
            self.pet.height = self.current_size.height() - change_value
            self.move(self.current_size.x() + change_value // 2, self.current_size.y() + change_value // 2)
            self.update()
